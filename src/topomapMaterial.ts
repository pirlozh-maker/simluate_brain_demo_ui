  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float value = weightTotal < 0.00001 ? 0.0 : weightedSum / weightTotal;
        value = clamp(value, minValue, maxValue);
        float t = clamp((value - minValue) / (maxValue - minValue), 0.0, 1.0);
        vec3 color = colormap(t);
        float alpha = smoothstep(0.02, 0.6, weightTotal / float(ELECTRODE_COUNT));

        gl_FragColor = vec4(color, 0.85 * alpha + 0.05);
      }
    `,
  });
};
